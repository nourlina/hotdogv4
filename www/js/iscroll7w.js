...
iScroll7.prototype = {
  ...
  if(target.tagName != 'SsELECT' && target.tagName != 'TEXTAREA' && target.tagName != 'INPUT' && target.tagName != 'CHECKBOX') {
 e.stopPropagation();//alert('kkk');
        }
		//e.stopPropagation();
 
 else { e.preventDefault(); //alert('45');
 }
 
 
		that.scrolling = true;		// This is probably not needed, but may be useful if iScroll7 is used in conjuction with other frameworks
...
// Expose iScroll7 to the world
window.iScroll7 = iScroll7;
})();